...
def get_log(loc, logfile_name):
    ...
    new_file_name = loc.split("testcase")[0] + "logs/" + logfile_name + '_' + current_time
    mv_command = 'mv {} {}'.format(latest_file, new_file_name)
    __run_command__(mv_command)
    return new_file_name

def verify_results(out,passed_file_name = passed_file_name, failed_file_name = failed_file_name):
...
parser.add_argument('--mul_tcs', dest='mul_tcs', action='store_true', help='Enable scripts to execute specific TCs from multiple driver files as mentioned in the input file')
parser.add_argument('--mul_driver', dest='mul_driver', action='store_true', help='Enable scripts to execute driver files mentioned in the input file')
parser.add_argument('--make_driver_bash', dest='make_driver_bash', action='store_true', help='makes bashfile to run entire driver file')
parser.add_argument('--make_tcs_bash', dest='make_tcs_bash', action='store_true', help='makes bashfile to run Tcs as mentioned in the input file')
parser.add_argument('--input_loc', dest='loc', action='store', help='location of the input file')

parser.add_argument('--pytest_params', dest='pytest_parms', action='store', help='string for pytest execution eg. to skip a Tc add params as "not <marker>" eg to execute only reboot markers from a driver file add "reboot" if reboot TCs needs to be skipped from the file add "not reboot')
...
if not (any ([args.mul_tcs, args.mul_driver,args.make_driver_bash, args.make_tcs_bash])) :
