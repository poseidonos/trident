...
parser.add_argument(
    "--verify_pos",
    action="store_true",
    help="this option will verify whether ibof os is installed or not",
)
parser.add_argument(
    "--verify_data_ip",
    action="store_true",
    help="this option will verify whether data ip is pingable b/w initiator and target using tcp or rdma",
)
parser.add_argument(
    "--get_data_ip",
    action="store_true",
...
class setup_config_tool:
    ...
    def check_nvme_ssd(self):
        ...
        match = [
            data
            for data in cmd_out
            if "The rule file removed" in data.strip()
            or "No need to remove" in data.strip()
        ]
        if not match:
            ...
            return False
        flag_2 = True
        reset_cmd = "{}/lib/{}/scripts/setup.sh reset".format(
            self.ibof_path, self.spdk_ver
        )
        ...
        for dev in dev_list:
            ...
            if int(bs_out[0].strip()) != 512:
                bs_summary = "device {} is having a block size of {}B which is not supported by ibofos ... Please re format to 512 ... ssd verification failed".format(
    ...
    def verify_pos(self):
        try:
            ...
            pass
        if not self.tar_ssh_obj:
            pos_ssh_summary = "Failed to take ssh for the machine: {} .... couldn't proceed further to verify POS".format(
                self.tar_ip
            )
        ...
        out = self.tar_ssh_obj.execute(
            "if test -d {}; then  echo ' exist'; fi".format(ibof_path)
        )
        ...
        if target_setup.status["ret_code"] is "fail":
            start_pos_summary = (
                "Starting of POS os failed ... POS command execution failed"
            )
        ...
        if target_setup.status["ret_code"] is "fail":
            scan_pos_summary = (
                "POS os failed to scan devices .... POS command execution failed"
            )
        ...
        if target_setup.status["ret_code"] is "fail":
            list_pos_summary = (
                "POS os failed to list the devices .... POS command execution failed"
            )
        ...
        while True:
            ...
            if choice in yes:
                if len(devs) >= 3:
                    ...
                    cnt_pos_summary = (
                        "Sufficient drives are not there for POS os to create array"
                    )
        ...
        if flag:
            ...
            if target_setup.status["ret_code"] is "fail":
                del_pos_summary = (
                    "failed to delete array .... POS command execution failed"
                )
    ...
    def get_mellanox_interface(self):
        try:
            ...
            pass
        if not self.tar_ssh_obj:
            ssh_tar_mlnx_summary = "Failed to take ssh for the machine: {} ... couldn't proceed further to test the kernel version of target machine ".format(
                self.tar_ip
            )
        ...
        try:
            ...
            pass
        if not self.init_ssh_obj:
            ssh_init_mlnx_summary_1 = "Failed to take ssh for the machine: {} ... couldn't proceed to further to test the kernel version of initiator machine".format(
                self.init_ip
            )
        ...
        if tar_mlx_ip[0] == False:
            tar_mlnx_summary = (
                "Mellanox interface is not configured on target machine {}".format(
                    self.tar_ip
                )
            )
            self.summary.append(tar_mlnx_summary)
        else:
            tar_mlnx_summary = ":::::::: Connected Mellanox information is {} on target machine {} :::::::: ".format(
                tar_mlx_ip[1], self.tar_ip
            )
            self.summary.append(tar_mlnx_summary)
        init_mlx_ip = init_ibof_obj.get_mellanox_interface_ip()
        if init_mlx_ip[0] == False:
            init_mlnx_summary = (
                "Mellanox interface is not configured on Initiator machine {}".format(
                    self.init_ip
                )
            )
            self.summary.append(init_mlnx_summary)
        else:
            init_mlnx_summary = "::::::::: Connected Mellanox information is {} on initiator machine {} :::::::::".format(
                init_mlx_ip[1], self.init_ip
            )
            self.summary.append(init_mlnx_summary)
