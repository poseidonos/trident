...
import lib.composable.vol_management as volmgmt
import lib.composable.io_management as iomgmt
import lib.composable.system_management as sysmgmt
import lib.composable.composable_core as libcore
from lib.composable.composable_core import _Data as data_mgmt
...
def test_vol_lc_stress_io_stress_io_sanity_system_sanity_6_initiator():
    try:
        ...
        test_object = {"vol_management": volmgmt, "io_management": iomgmt, "system_management": sysmgmt}
        ...
        for idx in range(1, total_phase+1):
            if str(idx) in por_phase:
                ...
                while True:
                    ...
                    for cn in range(test_dict["config"]["initiator"]):
                        assert libcore.npor_recover(target = pos, data_set=data_set[cn]) == True
                ...
                for cn in range(test_dict["config"]["initiator"]):
                    futures.append(
                        executor.submit(
                            getattr(
                                test_object[test_dict["validation"]["testcase"][cn]["lib"]],
        ...
        for cn in range(test_dict["config"]["initiator"]):
            if pos.client_handle[cn].ctrlr_list()[1] is not None:
                assert pos.client_handle[cn].nvme_disconnect(pos.target_utils.ss_temp_list) == True
