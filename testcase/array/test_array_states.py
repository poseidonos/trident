...
def test_array_states(num_array):
    try:
        ...
        for item in range(len(array_list)):
             list_array_obj.append(array(array_name = array_list[item], data_dict = data_dict, cli_history = pos.cli.cli_history))
        ...
        for subsystem in pos.target_utils.ss_temp_list:
            assert pos.client.nvme_connect(subsystem, pos.target_utils.helper.ip_addr[0], "1158") == True
        ...
        while True:
            logger.info("#################################################################################################")
            logger.info("---------------------------------------- LOOP COUNT : {} ----------------------------------------" .format(loop))
            logger.info("#################################################################################################")
