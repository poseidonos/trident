...
def test_Array_R10_R5_Rebuilding():
    ...
    try:
        ...
        for index, array in enumerate(array_list):
            assert pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=0 + index, num_data=4 + index,
                                            raid=raid_list[index], array_name=array)[0] == True
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="100gb",
                                                           vol_name="vol") == True
            ...
            assert pos.target_utils.mount_volume_multiple(array_name=array, volume_list=pos.cli.vols,
                                                          nqn_list=ss_list) == True
        ...
        for index, array in enumerate(array_list):
            assert pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=1 - index, num_data=5 - index,
                                            raid=raid_list[index - 1], array_name=array)[0] == True
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="100gb",
                                                           vol_name="vol") == True
            assert pos.target_utils.get_subsystems_list() == True
        ...
        assert pos.target_utils.device_hot_remove([pos.cli.array_info["posarray1"]['data_list'][0]])
        assert pos.cli.info_array(array_name="posarray1")[0] == True
...
def test_Array_R10_16drive_2Array():
    ...
    try:
        ...
        array_list = ["posarray1", "posarray2"]
        for index, array in enumerate(array_list):
            res = pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=0 + index, num_data=14,
                                           raid='RAID10', array_name=array)
            assert res[0] == True
            logger.info(json.dumps(res[1], indent=1))
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="5000gb",
                                                           vol_name="vol") == True
            assert pos.target_utils.get_subsystems_list() == True
            assert pos.cli.list_volume(array_name=array)[0] == True
            ss_list = [ss for ss in pos.target_utils.ss_temp_list if array in ss]
...
def test_Array_R10_Npor_R5():
    ...
    try:
        ...
        array_list = ["posarray1", "posarray2"]
        for index, array in enumerate(array_list):
            assert pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=0, num_data=4, raid='RAID10',
                                            array_name=array)[0] == True
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="500gb",
                                                           vol_name="vol") == True
            assert pos.target_utils.get_subsystems_list() == True
            assert pos.cli.list_volume(array_name=array)[0] == True
            ss_list = [ss for ss in pos.target_utils.ss_temp_list if array in ss]
        ...
        for index, array in enumerate(array_list):
            assert pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=1, num_data=5, raid='RAID5',
                                            array_name=array)[0] == True
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="500gb",
                                                           vol_name="vol") == True
...
def test_Array_R10_R5():
    ...
    try:
        ...
        array_list = ["posarray1", "posarray2"]
        for index, array in enumerate(array_list):
            res = pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=0, num_data=4, raid='RAID10',
                                           array_name=array)
            assert res[0] == True
            logger.info(json.dumps(res[1], indent=1))
            assert pos.cli.mount_array(array_name=array)[0] == True
        ...
        for index, array in enumerate(array_list):
            res = pos.cli.autocreate_array(buffer_name=f'uram{str(index)}', num_spare=1, num_data=5, raid='RAID5',
                                           array_name=array)
            assert res[0] == True
            logger.info(json.dumps(res[1], indent=1))
            assert pos.cli.mount_array(array_name=array)[0] == True
...
def test_Array_R10_Remove_Data_Spare():
    ...
    try:
        ...
        for array in list(pos.cli.array_dict.keys()):
            ...
            assert pos.target_utils.device_hot_remove([pos.cli.array_info[array]['spare_list'][0]])
            assert pos.cli.info_array(array_name=array)[0] == True
...
def test_Array_R10_QoS_256vol():
    ...
    try:
        ...
        for index, array in enumerate(array_list):
            ...
            logger.info(json.dumps(res[1], indent=1))
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=256, size="10gb",
                                                           vol_name="vol") == True
            assert pos.target_utils.get_subsystems_list() == True
            assert pos.cli.list_volume(array_name=array)[0] == True
            for volname in pos.cli.vols:
                assert pos.cli.create_volume_policy_qos(arrayname=array, volumename=volname, maxiops=10, maxbw=10)
            ss_list = [ss for ss in pos.target_utils.ss_temp_list if array in ss]
            assert pos.target_utils.mount_volume_multiple(array_name=array, volume_list=pos.cli.vols,
                                                          nqn_list=ss_list) == True
        for ss in pos.target_utils.ss_temp_list:
...
def test_Array_R10_QoS_2vol():
    ...
    try:
        ...
        for index, array in enumerate(array_list):
            ...
            logger.info(json.dumps(res[1], indent=1))
            assert pos.cli.mount_array(array_name=array)[0] == True
            assert pos.target_utils.create_volume_multiple(array_name=array, num_vol=2, size="1000gb",
                                                           vol_name="vol") == True
            assert pos.target_utils.get_subsystems_list() == True
            assert pos.cli.list_volume(array_name=array)[0] == True
            for volname in pos.cli.vols:
