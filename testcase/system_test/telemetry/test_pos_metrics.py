    ...
    for array in array_list:
        ...
        for vol in list(pos.cli.vols):
            ...
            assert pos.prometheus.get_volume_capacity_total(array_name=array, volume_name=vol) == str(
                pos.cli.volume_info[array][vol]['total_capacity']), "Total volume capacity does not match"
            time.sleep(20)
            ...
            assert pos.prometheus.get_volume_capacity_used(array_id=str(array_info['index']),
                ...
                pos.cli.vol_dict[vol]['total'] - pos.cli.volume_info[array][vol][
            ...
            assert pos.prometheus.volume_states[pos.prometheus.get_volume_state(array_name=array, volume_name=vol)] == \
                   pos.cli.volume_info[array][vol]['status'].lower(), "Volume status does not match"
    return True
    ...
    try:
        ...
        for array in arrays:
            ...
            if testcase == "T1":
                ...
                assert pos.prometheus.get_smart_stats(device_name=pos.cli.array_info[array]['data_list'][0]) == True
                ...
                vols = pos.cli.vols
                for vol in vols:
                    assert pos.cli.create_volume_policy_qos(volumename=vol,arrayname=array, maxiops=10000,maxbw=5000)[0] == True
    ...
    try:
        ...
        for array in arrays:
            for iter in range(3):
                ...
                assert pos.cli.info_array(array_name=array)[0] == True
                assert pos.prometheus.array_states[
                           pos.prometheus.get_array_state(uniqueid=pos.cli.array_info[array]['uniqueId'])] \
                       == pos.cli.array_info[array]['situation']
                assert pos.target_utils.array_rebuild_wait(array_name=array) == True
    except Exception as e:
        logger.info(f"Testcase failed due to {e}")
        pos.exit_handler(expected=False)
...
def test_telemetry_with_data_device_replacement(array_fixture,num_spare):
    ...
    try:
        ...
        assert pos.cli.info_array(array_name=arrays[0])[0] == True
        data_disk_list = pos.cli.array_info[arrays[0]]["data_list"]
        for iter in range(num_spare):
            assert pos.cli.replace_drive_array(device_name=pos.cli.array_info[arrays[0]]['data_list'][0],array_name=arrays[0])[0] == True
