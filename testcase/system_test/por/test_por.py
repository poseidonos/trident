...
def runFIO(io_mode, device_list,ops):
    ...
    if io_mode == True:
           assert pos.client.fio_generic_runner(device_list, fio_user_data=fio_write_line,IO_mode = io_mode)[0] == True
    else:
...
def test_por(writeback, numvol, numarray, iomode, spor):
    try:
        ...
        por_dict["array"]["pos_array"][0]["write_back"], por_dict["array"]["pos_array"][1]["write_back"] = writeback, writeback
        por_dict["volume"]["pos_volumes"][0]["num_vol"],por_dict["volume"]["pos_volumes"][1]["num_vol"] = numvol, numvol
...
def test_random_por():
    try:
        ...
        while True:
            ...
            logger.info(f"Remaining time for the test to be completed is {str(time_left)} minutes")
