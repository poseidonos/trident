...
def test_raid6_longetivity_npor_spor_journal_enabled(journal_setup_cleanup,por):
    ...
    pos = journal_setup_cleanup
    try:
        assert pos.pos_conf.journal_state(enable=True,
                                          update_now=True) == True
        assert pos_bringup(pos) == True
        assert multi_array_data_setup(data_dict = pos.data_dict,num_array = 2,raid_types = ("RAID6","RAID6"),num_data_disks=(4,4),num_spare_disk=(1,1),auto_create=(True,True),array_mount=("WT","WT")) == True
