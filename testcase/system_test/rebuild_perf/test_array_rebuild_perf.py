...
def test_set_rebuild_perf(array_fixture, test_param):
    ...
    try:
        ...
        fio_cmd = f"fio --size=200gb --ioengine=libaio --iodepth={qd} --rw={rw} --bs={bs} --direct=1 --name=test_{rw}"
...
rebuild_perf_impact_tests = {
    # Rebuild Perf, Block Size, Queue Depth, Work load
    "t0" : {"impact" : "lower",    "result": False},
    "t1" : {"impact" : "lowest",  "result": False},
    "t2" : {"impact" : "medium",  "result": True},
    "t3" : {"impact" : "lower",   "result": False},
    "t4" : {"impact" : "low",     "result": True},
    "t5" : {"impact" : "high",    "result": True},
    "t6" : {"impact" : "higher",  "result": False},
    "t7" : {"impact" : "highest", "result": False},
}

@pytest.mark.regression
@pytest.mark.rebuild_perf
@pytest.mark.parametrize("test_param", rebuild_perf_impact_tests)
def test_rebuild_perf_impact(array_fixture, test_param):
    """
    The purpose of this test is to create a RAID5 array. Create 2 volumes of 100 GB each. 
    Run IO of one of the type (randwrite, randread, write, read) in first volume. 
    During IO pefrom data Disk Hot Remove and verify disk is replaced. 
    ...
    """
    logger.info(
        f" ==================== Test : test_rebuild_perf_impact[{test_param}] ================== "
    )
    try:
        pos = array_fixture
        raid_type, mount_type, auto_create = "RAID5", "WT", True
        rpi_dict = rebuild_perf_impact_tests[test_param]
        num_vols = 2
        ...
        fio_cmd = "fio --size=100gb --ioengine=libaio --iodepth=128 --rw=write --bs=128k --direct=1 --name=test_write"
        ...
        # Array disk hot remove
        assert array_disk_remove_replace(pos, array_list, verify_rebuild=True) == True
        ...
        logger.info(f"Read  - {fio_read}       {fio_write}")
        
        # Rerun The FIO command
        out, async_block_io = pos.client.fio_generic_runner(
                nvme_devs, fio_user_data=fio_cmd, run_async=True)
        assert out == True
        ...
        fio_read_new  =  pos.client.fio_par_out["read"]
        fio_write_new =  pos.client.fio_par_out["write"]
        logger.info(f"FIO - Size : 100GB, IO Depth : 128, Block Size : 128k, RW : write")
        logger.info(f"Write Perf :   Before / After")
        logger.info(f"BW   - {fio_write['bw']} / {fio_write_new['bw']}")
        logger.info(f"IOPS - {fio_write['iops']} / {fio_write_new['iops']}")
