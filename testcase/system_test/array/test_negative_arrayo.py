...
def test_array_invalid_data_disk(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
...
@pytest.mark.regression
def test_create_array_invalid_commands(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
    logger.info(
...
@pytest.mark.regression
def test_array_less_data_drive(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=2):
    logger.info(
...
@pytest.mark.regression
def test_add_spare_without_array_mount(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
...
@pytest.mark.regression
def test_remove_spare_without_array_mount(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
    logger.info(" ==================== Test : test_add_spare_without_mount ================== ")
    try:
        pos = setup_cleanup_array_function
        assert negative_test_setup_function(pos, nr_data_drives) == True
        ...
        assert pos.cli.rmspare_array(array_name=array_name,device_name=spare_disk_list[0])[0] == False
        logger.info(
            " ============================= Test ENDs ======================================"
        )
    except Exception as e:
...
@pytest.mark.regression
def test_mnt_vol_stop_arrray_state(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
    logger.info(
...
@pytest.mark.regression
def test_rename_vol_stop_arrray_state(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
    logger.info(
        " ==================== Test : test_mnt_vol_fault_arrray_state ================== "
    )
    try:
...
def test_uram_creation_after_scan(setup_cleanup_array_function, nr_data_drives=3):
    logger.info(" ==================== Test : test_uram_creation_after_scan ================== ")
    try:
        pos = setup_cleanup_array_function
        assert negative_test_setup_function(pos, nr_data_drives) == True
        assert pos.cli.scan_device()[0] == True
        assert pos.cli.create_device(uram_name='uram2',bufer_size=8388608,strip_size=512,numa=0)[0] == True
        logger.info(
            " ============================= Test ENDs ======================================"
        )
    except Exception as e:
...
@pytest.mark.regression
def test_delete_create_array(setup_cleanup_array_function, raid_type="RAID5", nr_data_drives=3):
    logger.info(" ==================== Test : test_add_spare_without_mount ================== ")
    try:
        pos = setup_cleanup_array_function
        assert negative_test_setup_function(pos, nr_data_drives) == True
