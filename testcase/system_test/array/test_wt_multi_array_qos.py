    ...
    try:
        ...
        for index, array in enumerate(array_list):
            ...
            res = pos.cli.create_array(
                write_buffer=f"uram{str(index)}",array_name=array,data=data_disk_list,spare=None, raid_type=raid_type,
            ...
            assert pos.target_utils.mount_volume_multiple(array_name=array,
                                                          volume_list=pos.cli.vols, nqn_list=[ss_list[index]]) == True
        ...
        if IO =="File":
            for dev in pos.client.nvme_list_out:
                ...
                assert pos.client.fio_generic_runner(mount_point, fio_user_data=fio_cmd, IO_mode=False, run_async=True)[
                           0] == True
