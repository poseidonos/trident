...
def multi_io(pos, IO):
    ...
    if IO == "File":
        assert (
            pos.client.create_File_system(device_list=pos.client.nvme_list_out) == True
        )
        assert pos.client.mount_FS(device_list=pos.client.nvme_list_out)[0] == True

        pos.client.fio_generic_runner(
            list(pos.client.mount_point.values()), IO_mode=False
        )
        ...
        assert pos.client.fio_generic_runner(pos.client.nvme_list_out)[0] == True
