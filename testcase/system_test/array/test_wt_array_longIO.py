...
def test_wt_array_long_fileIO(setup_cleanup_array_function,  raid_type, nr_data_drives):
...
def test_wt_wb_array_long_fileIO(setup_cleanup_array_function, raid_type, nr_data_drives):
    ...
    try:
        ...
        assert (
            pos.client.nvme_connect(
                ss_list[0], pos.target_utils.helper.ip_addr[0], "1158"
            )
            == True
        )
        assert pos.client.nvme_list() == True
...
def test_wb_wt_array_long_fileIO(setup_cleanup_array_function, raid_type, nr_data_drives):
