...
class _Data:
    ...
    def add_subsystem(self, basename, serial = "IBOF00000000000001", model = "IBOF_VOLUME"):
        ...
        if not self.subsystem.get(basename):
            ...
            last_subsystem = self.subsystem[basename][len(self.subsystem[basename])-1].name
...
def subsystem_module(target, client, data_set, config_dict, action, phase=None):
    try:
        ...
        def create(basename):
            ...
            assert target.cli.create_subsystem(nqn_name = nqn_name, ns_count = "256", model_name = model_number)[0] == True
            assert target.target_utils.get_subsystems_list() == True
            assert target.cli.add_listner_subsystem(nqn_name = nqn_name, mellanox_interface = ip, port = port)[0] == True
        ...
        def connect(basename):
            ...
            assert client.nvme_connect(nqn_name = nqn_name, mellanox_switch_ip = ip, port = port) == True
        ...
        if action == "get_name":
            if not config_dict["phase"][phase]["nvmf_subsystem"]["connect"]["valid"]:
                return False        
            subsystem_name = config_dict["phase"][phase]["nvmf_subsystem"]["connect"]["basename"].split(",")
            subnqn_list = []
            ...
            if not config_dict["phase"][phase]["nvmf_subsystem"][action]["valid"]:
                return True        
            subsystem_name = config_dict["phase"][phase]["nvmf_subsystem"][action]["basename"].split(",")
            for basename in subsystem_name:
                if action == "create":
                    create(basename)
                elif action == "delete":
...
def volume_module(target, data_set, config_dict, action, phase = None):
    try:
        logger.info("Action={}, phase={}, valid_action? {}".format(action, phase, config_dict["phase"][phase]["volume"][action]["valid"]))
        ...
        def create(basename, number):
            ...
            for idx in range(int(number)):
                ...
                assert target.cli.create_volume(volumename = vol, size = size, array_name = array_name, iops = maxiops, bw = maxbw)[0] == True 

        def delete(basename, number):
            for idx in range(int(number)):
                vol = data_set.remove_volume(basename)
                assert target.cli.delete_volume(volumename = vol, array_name = array_name)[0] == True 
        
        def mount(basename, number, subsystem):
            nqnname = data_set.get_subsystem(subsystem)
            vol_list = data_set.get_volume(basename, number)
            for vol in vol_list:
                assert target.cli.mount_volume(volumename = vol, array_name = array_name, nqn = nqnname)[0] == True
            data_set.set_volume_state(basename = basename, number = number, nqnname = nqnname, state = "mount")

        def unmount(basename, number):
            vol_list = data_set.get_volume(basename, number)
            for vol in vol_list:
                assert target.cli.unmount_volume(volumename = vol, array_name = array_name)[0] == True
        ...
        def update(basename, number):
            ...
            for vol in vol_list:
                ...
                assert target.cli.create_volume_policy_qos(volumename = vol, arrayname = array_name, maxiops = iops, maxbw = bw, miniops = 0, minbw = 0)[0] == True
        ...
        for idx, (basename, number) in enumerate(zip(vol_basename, vol_number)):
            if action == "mount":
                subsystem = config_dict["phase"][phase]["volume"]["mount"]["nqnbasename"].split(",")[idx]
...
def npor_recover(target, data_set):
    try:
        ...
        for subsystem in total_subsystem:
            ...
            port = subsystem.port
            assert target.cli.create_subsystem(nqn_name = name, ns_count = "256", model_name = model)[0] == True
            assert target.cli.add_listner_subsystem(nqn_name = name, mellanox_interface = ip, port = port)[0] == True
        ...
        for vol in vol_list:
            ...
            if nqn_name == None: assert 0
            assert target.cli.mount_volume(volumename = vol.name, array_name = array_name, nqn = nqn_name)[0] == True
        return True    
