        ...
        while self.nss:
            ...
            for ctrl_id in ns_obj.ctrl_list:
                ...
                if not self.attach_detach_ns(attach=False, nvme_dev=nvme_dev,
                    ...
                    logger.error(
                        f"Failed to detach ns[{nsid}] ctrl[{ctrl_id}]")
                    return False
    ...
    def recover_test_devices(self, recovery_file) -> bool:
        ...
        for dev_name, dev_data in self.recovered_nvme_dev_data.items():
            ...
            if dev_sn in sys_dev_ns_dict.keys():
                ...
                logger.error(
                    f"Recovered test device {dev_name} does not exist")
                return False
    ...
    def prepare(self, hetero_setup_data: dict) -> bool:
        ...
        for index in range(end):
            ...
            if not dev.cleanup_ns():
                logger.error(
                    f"Failed to clean namespace for device {dev_name}")
                return False

            if not dev.setup_ns(ns_config_list=dev_config['ns_config']):
                logger.error(
                    f"Failed to setup namespace for device {dev_name}")
                return False
